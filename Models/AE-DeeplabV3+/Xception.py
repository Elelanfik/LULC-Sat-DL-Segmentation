...
from tensorflow.keras.layers import Input, Conv2D, BatchNormalization, ReLU, Dropout, Multiply, GlobalAveragePooling2D, Reshape, Dense, AveragePooling2D, UpSampling2D, concatenate
from tensorflow.keras.models import Model
from tensorflow.keras.applications import Xception
...
def DilatedSpatialPyramidPooling(input_tensor, dropout_rate=0.5):
    ...
    x2 = convolution_block(input_tensor, num_filters=256, kernel_size=3, dilation_rate=1, dropout_rate=dropout_rate)
    x2 = squeeze_excite_block(x2, dropout_rate=dropout_rate)

    x3 = convolution_block(input_tensor, num_filters=256, kernel_size=3, dilation_rate=6, dropout_rate=dropout_rate)
    x3 = squeeze_excite_block(x3, dropout_rate=dropout_rate)

    x4 = convolution_block(input_tensor, num_filters=256, kernel_size=3, dilation_rate=12, dropout_rate=dropout_rate)
    x4 = squeeze_excite_block(x4, dropout_rate=dropout_rate)

    x5 = convolution_block(input_tensor, num_filters=256, kernel_size=3, dilation_rate=18, dropout_rate=dropout_rate)
    x5 = squeeze_excite_block(x5, dropout_rate=dropout_rate)
    
    x6 = convolution_block(input_tensor, num_filters=256, kernel_size=5, dilation_rate=24, dropout_rate=dropout_rate)
    x6 = squeeze_excite_block(x6, dropout_rate=dropout_rate)
    ...
    concat = concatenate([x1, x2, x3, x4, x5, x6, pooling])
...
def DeeplabV3Plus(image_size, num_classes):
    ...
    input_b = xception.get_layer('block4_sepconv2_bn').output
    ...
    output_layer = Conv2D(num_classes, kernel_size=(1, 1), padding='same', activation='softmax')(x)

    model = Model(inputs=input_layer, outputs=output_layer, name='DeepLabV3Plus')
    return model
...
with strategy.scope():
    ...
    latest_checkpoint_path = "/home/rdadmin/Tsiyon/NEWONE/MODELSAVED/AEDEEPXCEP_best.weights.h5"
    try:
    ...
    best_checkpoint = ModelCheckpoint(
        filepath="/home/rdadmin/Tsiyon/NEWONE/MODELSAVED/1AEDEEPXCEP_best.weights.h5",
    ...
    last_checkpoint = ModelCheckpoint(
        filepath="/home/rdadmin/Tsiyon/NEWONE/MODELSAVED/1AEDEEPXCEP_latest.weights.h5",
        save_weights_only=True,
        verbose=1
    )
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/NEWONE/RESULT/AEDEEPXCEPLOSS.png')
plt.close()
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/NEWONE/RESULT/AEDEEPXCEPdice.png')
plt.close()
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/NEWONE/RESULT/AEDEEPXCEPJAC.png')
plt.close()
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/NEWONE/RESULT/AEDEEPXCEPIOU.png')
plt.close()
