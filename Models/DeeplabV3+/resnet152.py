...
def DilatedSpatialPyramidPooling(input):
    ...
    # Atrous block 1 (dilation_rate=1)
    ...
    atrous_block1 = Dropout(dropout_rate)(atrous_block1)

    # Atrous block 6 (dilation_rate=6)
    ...
    atrous_block6 = Dropout(dropout_rate)(atrous_block6)

    # Atrous block 12 (dilation_rate=12)
    ...
    atrous_block12 = Dropout(dropout_rate)(atrous_block12)

    # Atrous block 18 (dilation_rate=18)
    ...
    atrous_block18 = Dropout(dropout_rate)(atrous_block18)
...
def DeeplabV3Plus(image_size, num_classes):
    ...
    resnet152 = ResNet152(weights='imagenet', include_top=False, input_tensor=input)

    x = resnet152.get_layer('conv4_block6_out').output  # Updated layer name
    x = DilatedSpatialPyramidPooling(x)

    input_a = UpSampling2D(size=(image_size // x.shape[1], image_size // x.shape[2]), interpolation='bilinear')(x)
    input_b = resnet152.get_layer('conv2_block3_out').output  # Updated layer name
    ...
    return Model(inputs=input, outputs=output, name='DeepLabV3')  #MODEL TRAINING
...
with strategy.scope():
    ...
    earlystop = EarlyStopping(monitor='val_dice_coef_metric', patience=20, restore_best_weights=True, mode='max')
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.3, patience=10, mode='min', verbose=1)
    checkpoint = ModelCheckpoint(
        filepath="/home/rdadmin/Tsiyon/NEWONE/MODELSAVED/DEEPV3RES152.weights.h5",
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/NEWONE/RESULT/DEPRES152LOSS.png')
plt.show()
plt.close()
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/RESULT_FIGURE/DEEPLAB/256/DEPRES152dice.png')
plt.show()
plt.close()
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/RESULT_FIGURE/DEEPLAB/256/DEPRES152JAC.png')
plt.show()
plt.close()
...
plt.xticks(x_ticks)
plt.legend()
plt.savefig('/home/rdadmin/Tsiyon/RESULT_FIGURE/DEEPLAB/256/DEPRES152IOU.png')
plt.show()
plt.close()
